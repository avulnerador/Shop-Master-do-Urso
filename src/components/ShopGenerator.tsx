...
import { Sparkles, Save, Trash2, RefreshCw, Edit, Plus, X, Search, DollarSign } from 'lucide-react';
...
const ShopGenerator: React.FC = () => {
  const { 
    ...
    availableShopTypes, availableItemTypes 
  } = useStore();
      ...
      <div className="lg:col-span-4 space-y-6 flex flex-col h-full overflow-hidden">
        ...
        <div className="bg-surface border border-white/10 rounded-xl p-6 shadow-lg flex-shrink-0">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.type}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div>
                     <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.minItems}</label>
                     <input 
                ...
                <div>
                     <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.maxItems}</label>
                     <input 
                        type="number" min="0" max="50"
                        className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-white placeholder-gray-500"
                        value={maxItems}
            ...
            {/* System Filter */}
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.filters}</label>
              <div className="flex flex-wrap gap-2">
                {(['D&D 5e', 'Tormenta 20', 'Generic'] as SystemTag[]).map(sys => (
                  <button
                    key={sys}
        ...
        <AnimatePresence mode="wait">
          {currentShop ? (
            ...
            >
               ...
               {isEditMode && (
                   <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white text-xs px-3 py-1 rounded-full shadow-lg z-20 font-bold uppercase tracking-widest">
                       Editing Mode Active
                   </div>
               )}
              <div className={isEditMode ? "ring-2 ring-primary rounded-xl" : ""}>
                 <div className="relative">
                 ...
                 {isEditMode && (
                     <div className="mt-4 bg-black/30 border border-primary/30 p-4 rounded-xl">
                         <h4 className="text-primary text-xs font-bold uppercase mb-2">{t.editItems}</h4>
